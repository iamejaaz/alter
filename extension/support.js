...
const VERBS = {
  ...
  diagnose: (id) =>
    `Use the frappe-support-diagnosis skill in FAST MODE — quick (a handful of tool calls, ~30s). Do ONLY: read the WHOLE thread — the ticket description AND all replies (Communications) AND comments, because the real requirement is usually in the customer's LATEST replies, NOT the opening description (reading only the description is the #1 cause of a wrong verdict). State the actual ask in one line first. Then establish facts, quick code peek if needed, then CLASSIFY and answer. The BAR for calling it a 🔴 Bug is HIGH: the framework must MALFUNCTION — crash, lose/corrupt data, return a WRONG result, or violate its own documented/obvious contract. If you can't name which of those it is, it is NOT a bug. Critically: "the framework doesn't do X automatically" (e.g. doesn't sync a second field, doesn't auto-register something) is a CUSTOMISATION or enhancement, NOT a bug — if your fix would ADD behavior, it's not a defect; the answer is the hook/Server Script/setting that does X (note "could be a framework enhancement" at most). A deliberate skip in code is design, not oversight. DEFAULT to not-a-bug. Be DECISIVE — land flatly on ONE of: (a) the functionality DOESN'T EXIST → "Frappe doesn't do X; it's not a bug; to get X do <customisation>"; (b) a feature EXISTS and is BROKEN → 🔴 Bug, name the malfunction; (c) CONFIG → name the exact setting/state and fix. Do NOT hedge with "can't confirm, ask the customer to test whether Y also happens" — that's an experiment YOU run (trace to the decisive line or press Confirm on bench); only ask the customer for facts only they have (their script, their exact config). Trace to the line where behavior actually diverges — don't stop at "same code path". If it's not a bug, give the real answer and you're DONE. Only if it truly clears the bar, state the root cause and set Reproduced to "not run — press Confirm on bench". Do NOT version-triage / reproduce / search gh in fast mode. Follow Output + PROPORTIONALITY (SHORT). Write the VISIBLE answer in plain English like you're telling a non-technical teammate — NO file paths, line numbers, or function names in the visible part (put any code detail ONLY inside a collapsible Evidence drawer). Diagnose HD Ticket ${id}.`,
  draft: (id) =>
