...
const VERBS = {
  ...
  diagnose: (id) =>
    `Use the frappe-support-diagnosis skill and follow its method AND its Output format — especially the PROPORTIONALITY rule: keep it SHORT, match the length to the finding, and for a simple or low-severity ticket give just the few-line verdict with NO <details> blocks and no extra sections. Diagnose HD Ticket ${id} as a senior support engineer: read it via fr, find the code across ALL installed apps (ls apps/ first — never assume "custom"), triage which versions are affected (develop → v16 → v15, the customer's line), and check gh for an existing fix. REPRODUCE it via the skill's repro.sh helper (allowed even here — it runs on the local disposable benches); only if that version's bench isn't configured, say so and fall back to the trace. Don't assume you can't run a bench.`,
  draft: (id) =>
