...
export default function RoutinesPanel({ routines, onChange, onRunNow, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-[560px] max-h-[80vh] overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-900 p-5 shadow-2xl"
      ...
      >
        ...
        <div className="space-y-2 mb-5">
          {routines.length === 0 && <p className="text-sm text-zinc-500">No routines yet.</p>}
          {routines.map((r) => (
            <div key={r.id} className="rounded-lg border border-zinc-800 bg-zinc-800/40 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-zinc-500">every {r.everyMinutes}m</span>
                <button onClick={() => onRunNow(r)} className="text-[11px] text-indigo-400 hover:text-indigo-300">
                  Run now
                </button>
                <button onClick={() => remove(r.id)} className="text-zinc-500 hover:text-zinc-200">×</button>
              </div>
              <p className="mt-1 text-xs text-zinc-400 truncate">{r.prompt}</p>
              {r.lastRun && (
        ...
        <div className="space-y-2 border-t border-zinc-800 pt-4">
          ...
          <textarea
            ...
            rows={2}
            className="w-full resize-none rounded-lg bg-zinc-800 border border-zinc-700 px-3 py-2 text-sm focus:outline-none focus:border-indigo-500"
          />
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-400">Every</label>
            <input
              type="number"
              min={1}
              value={every}
              onChange={(e) => setEvery(Math.max(1, Number(e.target.value)))}
              className="w-20 rounded-lg bg-zinc-800 border border-zinc-700 px-2 py-1.5 text-sm focus:outline-none focus:border-indigo-500"
            />
