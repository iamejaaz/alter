...
extension agent may run this too** (its one allowed bench command); write the script into your scratchpad directory (the path in your system prompt; writes there need no approval, /tmp does) and pass the path — it runs as one unit, so functions and try/except work:
```sh
# <scratchpad>/repro.py
import frappe
reproduced = False
...
```
```sh
~/.claude/skills/frappe-support-diagnosis/scripts/repro.sh develop <scratchpad>/repro.py
```
