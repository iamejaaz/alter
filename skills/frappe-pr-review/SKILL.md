...
- **Before you say something is missing, prove it is missing.** Every "there is no guard", "this is not handled", "no test covers this", "it should also do X" is a claim about absence, and absence is the easiest thing to get wrong. Grep the file, the caller, the parent class, the base method, the decorator and the test file before you write it, and say where you looked. An ask for something that already sits three lines below, in the superclass, or in a `validate` hook is the fastest way to lose the author.
- **Another reviewer's finding gets the same test.** Greptile, Copilot or a human saying something is broken or missing is a claim, not a fact. Confirm it against the code before you echo it, defer to it, or build a finding on top of it, and say plainly when it does not hold. A false finding you repeat becomes yours.
...
- **Three sentences. Sixty words. That is the ceiling, and it beats every other rule in this section.** Nobody reads a paragraph on a diff line. If it does not fit, you are trying to say too much: cut the evidence, never the ask. One concern per comment, so a second concern is a second comment on its own line, or it is not worth raising.
- Where more than one shape is reasonable, propose **one** and ask, in a single clause: "Should we clear the flag instead?". Never lay out both options with what each buys, and never end with "which do you prefer?" — that is a design meeting, not a review comment. Where the finding rests on an assumption, say so in three words: "unless there is a reason to keep it", "does that hold?". The author knows things you do not.
- **Say what is wrong with the line in front of you, never what the codebase "always" does.** A house rule stated as an absolute is usually false and always reads as a lecture. This repo keeps comments that explain a non-obvious workaround (`code_review.md`: "Comments explain why, never what"), so "the codebase is comment-free" is both wrong and preachy. Flag the comment itself: it repeats what the code says, so it is not needed. Same for any other convention — name what this line does, not a law.
- **Everything you learned while verifying stays out.** No "I checked a develop bench", no row counts, no list of six affected doctypes when one names the problem, no note on what a migrate does or does not hide. That belongs in the section 6 result block, which the maintainer reads. The comment carries the conclusion and the anchor, nothing else.
...
- Simple English, short sentences, one clause each. No idioms. Do not stack clauses behind a dash or a semicolon: start a new sentence, or cut it. Someone who is not a native speaker must read it once and know what to do.
...
A real one that was too long, and what it should have been:

> ✗ This validates stored metadata, not just what the caller is changing. On a site with ERPNext, adding a Custom Field to Task (or Employee, Department, BOM, Purchase Invoice, Sales Invoice Item) saves fine on develop, but with this PR it fails with "…" — `Custom Field.on_update` runs `validate_fields_for_doctype` on the whole doctype. I checked a develop bench: 10 DocField rows still carry the flag and only one of them is in frappe; the `in_migrate` skip hides the error during migrate but never clears the rows, so the site stays broken afterwards. One option is to clear the flag when it comes from existing metadata and throw only when the incoming change sets it; another is to keep the throw and ship a patch that resets the flag. Which do you prefer?

> ✓ On a site with ERPNext, adding a Custom Field to Task saves on develop but fails here, because `Custom Field.on_update` validates the whole doctype and old rows still carry the flag. Should we clear it when it comes from existing metadata, and throw only when the change sets it?

Same finding, same anchor, same ask. The bench counts, the six doctype names and the second option went to the result block.
